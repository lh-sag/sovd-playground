...
use crate::convert::{data_value_to_rest, parse_categories, value_metadata_to_models};
...
pub(super) async fn list_data_resources(
    ...
    let data_items = data_service
        .list(&entity_id, categories, groups)
        .await
        .map_err(crate::response::ApiError::from)?;
    debug!(component_id = %component_id, count = data_items.len(), "Data service list returned items");
    let items = value_metadata_to_models(data_items);
    let response = DataResourceResponse { items: items.clone() };
    debug!(component_id = %component_id, count = items.len(), "Return DataResourceResponse");
...
pub(super) async fn set_data_value(
    ...
    match Ok::<(), crate::response::ApiError>(()) {
        Ok(()) => Ok(HttpResponse::NoContent().finish()),
        Err(data_error) => Err(data_error),
    }
}
...
pub(super) async fn list_data_categories(
    ...
    let categories = data_service
        .list_categories(&entity_id)
        .await
        .map_err(crate::response::ApiError::from)?;

    let response = sovd_models::data::DataCategoryResponse {
        items: categories
            .into_iter()
            .map(|cat| sovd_models::data::DataCategoryInformation {
                item: crate::convert::diagnostic_category_to_models(cat.item),
                category_translation_id: cat.category_translation_id,
            })
            .collect(),
    };
...
pub(super) async fn list_data_groups(
    ...
    let groups = data_service
        .list_groups(&entity_id, category)
        .await
        .map_err(crate::response::ApiError::from)?;

    let response = sovd_models::data::DataGroupResponse {
        items: groups
            .into_iter()
            .map(|group| sovd_models::data::ValueGroup {
                id: group.id,
                category: crate::convert::diagnostic_category_to_models(group.category),
                category_translation_id: group.category_translation_id,
                group: group.group,
                group_translation_id: group.group_translation_id,
            })
            .collect(),
    };
