...
# Certificate generation and testing script for sovd-gateway HTTPS with client certificates
...
openssl req -new -x509 -days "$VALIDITY_DAYS" -key ca-key.pem -sha256 -out ca-cert.pem -subj "/C=DE/O=SOVD/CN=SOVD CA" -addext "basicConstraints=critical,CA:TRUE" -addext "keyUsage=critical,digitalSignature,keyCertSign,cRLSign"

# Server
openssl genrsa -out server-key.pem 4096
openssl req -subj "/C=DE/O=SOVD/CN=127.0.0.1" -sha256 -new -key server-key.pem -out server.csr
openssl x509 -req -days "$VALIDITY_DAYS" -in server.csr -CA ca-cert.pem -CAkey ca-key.pem -out server-cert.pem -CAcreateserial -extfile <(echo "basicConstraints=CA:FALSE"; echo "keyUsage=critical,digitalSignature,keyEncipherment"; echo "extendedKeyUsage=serverAuth"; echo "subjectAltName=IP:127.0.0.1,IP:::1,DNS:localhost")

# Client
openssl genrsa -out client-key.pem 4096
openssl req -subj "/C=DE/O=SOVD/CN=test-client" -new -key client-key.pem -out client.csr
...
echo "cargo run --bin sovd-gateway -- --url \"$URL\" --cert '$CERT_DIR/server-cert.pem' --key '$CERT_DIR/server-key.pem' --cacert '$CERT_DIR/ca-cert.pem'"
echo ""
