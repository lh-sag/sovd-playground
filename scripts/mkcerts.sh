...
cargo run -q --bin sovd-gateway --features "openssl" -- \
    --url "$URL" \
...
echo "cargo run --bin sovd-gateway --features \"openssl,jsonschema-schemars\" -- --url \"$URL\" --cert '$CERT_DIR/server-cert.pem' --key '$CERT_DIR/server-key.pem' --cacert '$CERT_DIR/ca-cert.pem'"
echo ""
