...
mod tests {
    ...
    fn test_list_data_items_filtering() {
        ...
        let ident_items = resource.list_data_items(&["identData".to_string()], &[]);
        assert_eq!(ident_items.len(), 1);
        assert_eq!(ident_items[0].id, "id1");

        let current_items = resource.list_data_items(&["currentData".to_string()], &[]);
        ...
        let engine_current = resource.list_data_items(&["currentData".to_string()], &["engine".to_string()]);
