...
async fn serve() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(url) = &url {
        // On Unix: try unix_socket first
        #[cfg(unix)]
        let unix_listen = args.unix_socket.clone().map(|s| format!("unix:/{s}"));

        #[cfg(not(unix))]
        let unix_listen: Option<String> = None;

        // Shared TCP socket logic
        let listen = if let Some(unix_socket_listen) = unix_listen {
            unix_socket_listen
        } else {
            let host = url
                .host_str()
                .ok_or("URL must have a host (e.g., http://localhost:9000/sovd)")?;
        ...
        servers.push(config::ServerConfig {
            name: "Command Line Server".to_string(),
            listen,
            server_name: vec![],
            base: url.path().to_string(),
            auth: args.auth_jwt.as_ref().map(|path| config::AuthConfig {
                jwt_public_key_path: path.clone(),
            }),
            ssl: if url.scheme() == "https" {
                Some(fallback_ssl.clone())
            } else {
                None
            },
        });
    ...
    let vendor_info = OpenSovdInfo {
        ...
        name: servers
            .first()
            .map_or_else(|| "SOVD Gateway".to_string(), |s| s.name.clone()),
    ...
    for server_cfg in &servers {
        ...
        let protocol = if server_cfg.ssl.is_some() { "https" } else { "http" };
        tracing::info!(
            target: TARGET,
            server_name = ?server_cfg.name,
            protocol = ?protocol,
            listen_addr = ?actual_addr,
            base_path = ?server_cfg.base,
            "Add server endpoint"
        );
